...
func TestTxSuccess(t *testing.T) {
	...
	txManager.DoWithTx(t.Context(), func(ctx context.Context) error {
		// add new user in tx
		id, err := userRepo.Create(ctx, &dto.UserCreate{Email: "test@mail.ru", PasswordHash: "123"})
		require.NoError(t, err)
		require.Equal(t, 1, id)
		// check if new user was added
		verifyUsersCount(t, t.Context(), (*extractTx(ctx)), 1)
		id, err = userRepo.Create(ctx, &dto.UserCreate{Email: "test1@mail.ru", PasswordHash: "123"})
		require.NoError(t, err)
